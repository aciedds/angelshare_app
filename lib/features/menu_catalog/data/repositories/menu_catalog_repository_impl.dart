...
class MenuCatalogRepositoryImpl implements MenuCatalogRepository {
  ...
  Future<DataState<List<String>>> getCategories() async {
    try {
      ...
      return const DataState.success(['Ordinary Drink', 'Cocktail', 'Shake', 'Cocoa', 'Shot']);
    } catch (e) {
      return DataState.failure(e is Exception ? e : Exception(e.toString()));
    }
  }
  ...
  Future<DataState<DrinkDetailEntity>> getCocktailDetail({required String id}) async {
    try {
      try {
        ...
        if (cached != null) {
          final domainDetail = DrinkDetailEntity(
            id: cached.idDrink,
          ...
          return DataState.success(domainDetail);
        }
      } catch (_) {
